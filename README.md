...
지침 설계형), 사례(`scenario`), 과업(`task`), AI 채팅 페르소나 지시문(`aiSystemPrompt`),
그리고 `rubric`(이 문항에서 좋은 답변이 다뤄야 할 내용 — 채점 시 AI에게 참고 맥락으로만
전달되고, 실제 점수는 항상 아래 6번의 5개 역량 기준으로 매겨집니다)으로 구성됩니다. 새
영역을 추가하려면 같은 형식으로 문항 객체를 배열에 추가하면 되고, 선택 화면의 영역 버튼은
문항 데이터의 `domain` 값을 기준으로 자동 생성됩니다(별도 코드 수정 불필요).

## 6. 채점 방식

제출(`/api/submit`) 시 문항마다 AI(Gemini/Groq)에게 사례·과업·문항별 참고 rubric·응시자
최종 답안·(해당 문항의) AI 대화 기록을 전달해 채점을 요청합니다. 모든 문항은 항상 동일한
**5개 역량 기준**(`data/questions.js`의 `COMPETENCIES`)으로 채점되어, 문항이 달라도 역량별
결과를 하나의 막대그래프로 합산·비교할 수 있습니다.
