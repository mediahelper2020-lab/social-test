...
- 제출 후: **AI가 실제로 채점**하여 총점 + 등급(A+~F)을 바로 보여줍니다. 문항마다
  평가기준 4개 + **개인정보 비식별 처리 여부**(공통 항목, 0~5점)까지 세분화해서 채점하고,
  각 기준별 점수와 채점 이유를 함께 표시합니다. 답안/채점결과/AI 대화기록은 다운로드(.txt)할 수
  있습니다.
...
제출(`/api/submit`) 시 문항마다 AI(Gemini/Groq)에게 사례·과업·평가기준·응시자 답안을 전달해
채점을 요청합니다. 평가기준은 문항별 `rubric` 4개 + 모든 문항에 공통으로 적용되는
**개인정보 비식별 처리** 기준(`data/questions.js`의 `PRIVACY_CRITERION`) 1개, 총 5개이며
각 기준을 0~5점으로 채점합니다(문항당 최대 25점). 문항 수만큼 합산해 총점·백분율을 계산하고,
아래 기준으로 등급을 매깁니다.
